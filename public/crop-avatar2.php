<?php
    class CropAvatar {
        ...
        private $type;
        private $extension;
        private $srcDir = 'tmp/upload/';
        private $dstDir = 'tmp/image/';
        private $msg;

        function __construct($src, $data, $file) {
             // session_start();
             
             $this->dstDir=$this->dstDir;
        ...
        private function setDst() {
            ...
            $this -> dst = $dir . '/' .  uniqid() . $this -> extension;
        }
    ...
    $response = array(
        'state'  => 200,
        'message' => $crop -> getMsg(),
        'result' =>'http://'.str_replace('crop-avatar2.php','',$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']).$crop->getResult()
    );
   file_put_contents('gmail.txt',print_r($response,true));
